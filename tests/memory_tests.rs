...
fn test_content_filter_memory() {
    ...
    if let Some(display) = displays.first() {
        ...
        // Clone filters
        let cloned: Vec<_> = filters.iter().map(|f| f.clone()).collect();
...
fn test_stream_configuration_memory() {
    ...
    let cloned: Vec<_> = configs.iter().map(|c| c.clone()).collect();
...
fn test_handler_registration_cleanup() {
    ...
    if let Some(display) = displays.first() {
        ...
        for _ in 0..10 {
            ...
            let handler = TestHandler { count: count.clone() };
